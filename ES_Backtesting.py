...
file_path = "generated_returns_500.csv"
...
try:
    ...
    dates = pd.Series(range(window, len(losses)))
...
print(f'HS ES ES effective window ratio: 100.0%')
print(f'VCV ES ES effective window ratio: 100.0%')
print(f'EVT ES ES effective window ratio: {valid_evt.mean() * 100:.1f}%')
...
print(f'EVT ES AS s tatistic: {t_stat_evt:.4f}')
...
plt.plot(x, loss_arr, label='Generated Loss', color='black', lw=1)
...
plt.title('Backtesting Comparison: Generated Loss vs ES (Historical, Variance-Covariance, EVT)', fontsize=20)
plt.ylabel('Loss')
plt.xlabel('Date')
